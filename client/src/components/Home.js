...
function Home() {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.put(`http://localhost:5000/api/update/${modalImage._id}`,
        {
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this image?')) {
      try {
        ...
        const response = await axios.delete(`http://localhost:5000/api/delete/${modalImage._id}`, {
          headers: {
            'Authorization': localStorage.getItem('authtoken')
          },
            ...
            {filteredImages.map((image) => (
              <div key={image._id} className="col">
                <div className="card h-100" onClick={() => openModal(image)}>
                  <img
                    src={`http://localhost:5000/api/file/${image.filename}`}
              ...
              <div className="modal-dialog modal-lg">
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    <img
                      src={`http://localhost:5000/api/file/${modalImage.filename}`}
